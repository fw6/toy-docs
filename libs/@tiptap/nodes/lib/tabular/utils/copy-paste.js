 ...
 *
 * @param {Slice} slice
 * @returns {CellSelectionRect | null}
 */
 ...
 * they stick out.
 *
 * @param {CellSelectionRect} rect
 * @param {number} newWidth
 * @param {number} newHeight
 * @returns {CellSelectionRect}
 */
export function clipCells({ width: currentWidth, height: currentHeight, rows: currentRows }, newWidth, newHeight) {
 ...
 * @param {NonNullable<Dispatch>} dispatch
 * @param {number} tableStart
 * @param {import("@tiptap/pm/tables").Rect} rect
 * @param {CellSelectionRect} cells
 */
export function insertCells(state, dispatch, tableStart, rect, cells) {
    /** @type {PMNode | null | undefined} */
    let table = state.doc;
...
function growTable(tr, map, table, start, width, height, mapFrom) {
    ...
    if (height > map.height) {
        ...
        for (let i = 0, k = (map.height - 1) * map.width; i < Math.max(map.width, width); i++) {
            ...
            const newCell = header ? emptyHead : empty;
            newCell && cells.push(newCell);
        }
